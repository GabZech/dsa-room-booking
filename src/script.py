...
### ADD LANDING PAGE WHERE STUDENT CHOOSES TO EITHER BOOK A ROOM OR CANCEL A BOOKING
df = pd.read_csv("../data/processing/rooms.csv", dtype={"name": str})
...
df = pd.read_csv("../data/processing/rooms.csv", dtype={"name": str}) # re-read dataframe to avoid another user having already booked a place in same room during this time (thus preventing overbooking)
...
if df.iloc[chosen_room_index]["available_places"] >= booking.places: # if still available places
    ...
    df.at[chosen_room_index, "available_places"] = df.iloc[chosen_room_index]["available_places"] - booking.places 
    df.to_csv("../data/processing/rooms.csv", index = False)
    ...
    df_bookings = pd.read_csv("../data/processing/bookings.csv", dtype={"room_name": str})
...
def chooseBooking(df):
    ...
    while True:
        ...
        if chosen_booking not in [*range(0, len(room_names_list), 1)]: 
            ...
            df_rooms = pd.read_csv("../data/processing/rooms.csv", dtype={"name": str})
            ...
            df_bookings = pd.read_csv("../data/processing/bookings.csv")
